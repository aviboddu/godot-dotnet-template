...
public partial class InputSetting : Container
{
    ...
    private readonly Lazy<PackedScene> _inputActionPackedScene =
        new(() => (PackedScene)ResourceLoader.LoadThreadedGet(INPUT_ACTION_SETTING_PATH));
    ...
    public override void _Ready()
    {
        ...
        this.CheckedConnect(CanvasItem.SignalName.VisibilityChanged,
                            Callable.From(LoadActions)); // Create nodes only if user opens input menu
    }
    ...
    private void LoadActions()
    {
        foreach (StringName action in InputManager.GetCustomActions())
        {
            ...
            if (InputMap.ActionGetEvents(action).Any(e => e is InputEventMouseMotion || e is InputEventJoypadMotion))
                continue;
            InputActionSetting actionSetting = _inputActionPackedScene.Value.Instantiate<InputActionSetting>();
            AddChild(actionSetting);
            actionSetting.Action = action;
            actionSetting.CheckedConnect(InputActionSetting.SignalName.ChangedEvent,
                                         Callable.From<InputEvent, InputEvent>(PreventConflicts));
        }
